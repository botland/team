...
1. **Git is authoritative.** After test-writer, new dirty paths must sit under `test_root` (plus `.team/work/`). After implementer, under `code_root`. Agent `paths_touched` is ignored for the fence. A hop may not mutate paths that were already dirty when the run started. Files this apply dirtied stay writable on later hops (repair, later `--seq` classes) if they sit under that hop's root.
