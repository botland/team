...
| L10 | A warm chain trusts resume semantics nothing here executes | 4 | `--warm` resumes a session across same role+runtime+capability hops. Capability must match precisely because a resumed hop re-declaring different tool filters is the vendor behaviour `tests/support/*_argv.py` only models (open class L2), so the gate/write pair stays cold by construction. `WarmSessionParityTests` pins that both adapters agree on cold vs warm spelling, and `test_warm_and_cold_produce_the_same_artifacts` pins that dropping any link changes nothing -- the chain is an accelerator, never the channel. | **guarded** — the equivalence test holds it; only a smoke hop against the real binaries closes L2 underneath it |
...
| L8 | The review surface had no size law | 4 | `ignored_untracked` already says a generated tree is not review material, but it delegates the judgment to the target repo's `.gitignore`: a repo that does not ignore `coverage-html/` handed every hop an 11.1 MB `apply.patch` (2.8M tokens, 442 files, ~90% build output). `budget_sections` / `budget_patch_text` now cap the bytes a hop is *handed*; `porcelain_paths` and the write fence are untouched, and omitted paths stay in `apply-names.txt` / `names.txt` with a header naming them. Real case: 11.1 MB → 524 KB. `ReviewSurfaceBudgetTests` + `ApplySurfaceBudgetTests` pin that the fence does not shrink with the patch. | **guarded** — the cap is a number, so an under-budget generated tree still costs its bytes; rung 2 is the target repo owning what is reviewable |
