...
- Files are the protocol. Phases communicate through `.team/work/<slug>/` in the **target** repo. Each model hop is a new session **unless an explicitly enabled warm chain is live** (`--warm` / `TEAM_WARM` / `[run] warm`, all through `parse_bool`; off by default). A chain resumes only when the whole argv agrees — same role, runtime, capability **and** the `write_tool_path_filters` allow/deny sets themselves, since a replan can move the roots mid-run — in one process, and only after a hop that succeeded *in full* (a chain is consumed on read and re-armed at the single exit where nothing raised; `result.success` is the runtime's verdict, not the hop's), and only while the session's context is under `WARM_CONTEXT_CEILING` — past that a turn re-sends more than a cold hop costs. Files are the protocol either way: every prompt stands alone and every artifact is written, so any link can be dropped and rerun cold with the same result. That equivalence is a contract test, not an intention. Never make a chain the channel. `team resume` continues the pipeline, not the model — a resumed run has no live session and always starts cold. Tree inventory lives in `census.md`, written once **per repo state**: the first hop to emit `census_markdown` writes it, and it is cached at `.team/census/<head>.md` with a sidecar recording the dirty set, so a later slug at the same HEAD starts holding it. A reused map names what has moved under it since — by **content id**, so a path dirty before and dirty now with different bytes is named; a cache with no sidecar is not reused, because staleness that cannot be computed is not staleness that is absent. `.team/census` is deliberately **not** fence-exempt: it is durable and repo-wide, so anything a hop could leave there would be an input to every later run's prompts. The orchestrator declares the paths it writes itself instead (`_note_orchestrator_write`, path + content id), so the fence bills a hop for what the hop wrote. `.team/work` remains the one exempt root — per-slug and ephemeral. Later hops read the census; they do not recensus. Judgments stay in `review.md` / `guardian.md` / `design.md`. Spend lives in `usage.jsonl` / `usage.md` (orchestrator; provider envelope only — omitted `$` is unknown, never free).
